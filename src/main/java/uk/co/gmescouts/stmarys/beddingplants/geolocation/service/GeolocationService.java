...
public class GeolocationService {
	...
	public byte[] plotPointsOnGoogleMapImage(@NotNull final Set<LatLng> points, @NotNull final ImageFormat imageFormat,
		...
		LOGGER.debug("Generating Google Map Image of format [{}] and type [{}] for [{}] Points", imageFormat, staticMapType, points.size());

		byte[] imgData = null;

		if (!points.isEmpty()) {
			// generate Markers from points
			final Markers markers = new Markers();
			markers.size(markersSize);
			markers.color(markersColour);
			points.stream().filter(Objects::nonNull).forEach(markers::addLocation);

			// size of markers in image (from config)
